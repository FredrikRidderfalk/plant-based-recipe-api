...
router.get("/", async (req, res) => {
  try {
    const allRecipes = await find(); // .find() will find all the recipes. Mongoose has a lot of built-in methods to apply here, like .limit(5) will limit the posts returned to the first 5 posts.
    res.json(allRecipes);
  } catch (err) {
